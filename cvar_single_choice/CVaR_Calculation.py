...
def calc_cvar(R,y):
    ...
    cvar_leq = np.mean(R[R<=var])
    return(var,cvar,cvar_leq)

# calculating CVaR the other way
def cvar_loss(var,R,alpha):
    Rpos = R-var
    Rpos[Rpos<0]=0
    cvar = var + (1/alpha)*np.mean(Rpos) # trying to maximize that threshold.
    return(cvar)

def calc_cvar_optim(R,alpha):
    '''for a single alpha'''

    var0 = [np.mean(-1*R)]
    bnds = [(np.min(-1*R),np.max(-1*R))]
    results = minimize(cvar_loss, var0, args=(-1*R,alpha),bounds=bnds)
...
def find_CVaR_SLSQP(alpha,P_dist,Z_values):
    '''
    Inputs:
    ...
    def obj_fun(Xi,Z_values,P_dist):
        return(np.sum((-1*Xi*Z_values*P_dist)))

    # bounds
    bnds = tuple(((0.0,1.0/alpha) for i in range(len(Z_values))))

    # initial values
    Xi_init = np.random.uniform(0.0,1.0/alpha,len(Z_values))
